...
app.post("/api/admin/login", (req, res) => {
  if (req.body.username === "admin" && req.body.password === "admin123") res.json({ success: true, token: "sukoon_admin_secure_token" });
  else res.status(401).json({ error: "गलत यूज़रनेम या पासवर्ड!" });
});
app.get("/api/admin/dashboard", (req, res) => {
  res.json({ users: dbData.registeredUsers, totalPosts: dbData.posts.length, totalStories: dbData.stories.length, activeStreams: liveStreams.length, activeNow: io.engine.clientsCount });
});
app.post("/api/admin/approve-creator", (req, res) => {
  const user = dbData.registeredUsers.find(u => u.email === req.body.email);
  if (user) { user.isApprovedCreator = true; saveDB(); res.json({ success: true }); } else res.status(404).json({ error: "यूज़र नहीं मिला!" });
});
app.post("/api/admin/delete-user", (req, res) => {
  dbData.registeredUsers = dbData.registeredUsers.filter(u => u.email !== req.body.email); saveDB(); res.json({ success: true });
});
...
io.on("connection", (socket) => {
  ...
  io.emit("global_online_count", io.engine.clientsCount); // 🔥 LIVE COUNTER
  ...
  socket.on("get_nearby_users", ({ maxDistanceKM }) => {
    ...
    for (const sid in onlineLoggedInUsers) {
      ...
      if (other.lat && other.lon) {
        // simple distance logic omitted for code brevity
        list.push({ socketId: other.socketId, email: other.email, channelName: other.channelName, avatar: other.avatar, distanceKM: 10 });
      }
    }
  ...
  socket.on("add_friend", ({ myEmail, partnerSocketId }) => {
    ...
    if(partner && myEmail && partner.email !== myEmail) {
      ...
      if(!dbData.userConnections[myEmail].find(c => c.email === partner.email)) {
  ...
  const cleanup = () => {
    ...
    if (roomId && roomId.startsWith("audio_")) {
      if (audioRoomsHub[roomId]) { audioRoomsHub[roomId].participants = audioRoomsHub[roomId].participants.filter(x => x.socketId !== socket.id); socket.to(roomId).emit("audio_user_left", socket.id); if (audioRoomsHub[roomId].participants.length === 0) delete audioRoomsHub[roomId]; io.emit("audio_rooms_list", Object.values(audioRoomsHub)); }
    ...
    io.emit("global_online_count", io.engine.clientsCount); // 🔥 LIVE COUNTER UPDATE ON DISCONNECT
  };
