...
app.post("/api/user/follow-toggle", (req, res) => {
  ...
  saveDB();
  res.json({ success: true, isFollowing: !isFollowing, targetFollowersCount: dbData.follows[targetEmail].followers.length });
});
...
function attemptSmartMatch(user) {
  ...
  if (index !== -1) return waitingPool.splice(index, 1)[0];
  return null;
}
...
function saveMutualConnection(email1, name1, email2, name2) {
  ...
  if (idx1 === -1) dbData.userConnections[email1].unshift({ peerEmail: email2, peerName: name2 || email2.split('@')[0], isFriend: false, isBlocked: false, lastChatAt: timeStr });
  else dbData.userConnections[email1][idx1].lastChatAt = timeStr;

  const idx2 = dbData.userConnections[email2].findIndex(c => c.peerEmail === email1);
  if (idx2 === -1) dbData.userConnections[email2].unshift({ peerEmail: email1, peerName: name1 || email1.split('@')[0], isFriend: false, isBlocked: false, lastChatAt: timeStr });
  else dbData.userConnections[email2][idx2].lastChatAt = timeStr;
  saveDB();
}
...
io.on("connection", (socket) => {
  ...
  socket.on("register_user_presence", ({ email, channelName, avatar, lat, lon }) => {
    onlineLoggedInUsers[socket.id] = { socketId: socket.id, email, channelName: channelName || email.split('@')[0], avatar: avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${email}`, lat: lat || null, lon: lon || null };
  ...
  socket.on("get_my_connections", ({ email }) => {
    ...
    const enriched = history.map(conn => {
      ...
      return { ...conn, avatar: reg?.avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${conn.peerEmail}`, isOnline: onlineEmails.has(conn.peerEmail), isFriend: !!conn.isFriend, isBlocked: !!conn.isBlocked };
    });
  ...
  socket.on("send_nearby_request", ({ toSocketId }) => {
    ...
    if (sender && io.sockets.sockets.get(toSocketId)) {
      io.to(toSocketId).emit("receive_chat_request", { fromSocketId: socket.id, fromName: sender.channelName, fromAvatar: sender.avatar, fromEmail: sender.email });
    }
  });
  ...
  socket.on("join_stream", ({ streamId }) => {
    ...
    if (stream) { stream.viewersCount = (stream.viewersCount || 0) + 1; io.to(streamId).emit("stream_viewers_updated", { count: stream.viewersCount }); io.to(stream.socketId).emit("viewer_joined", { viewerSocketId: socket.id }); }
  });
  socket.on("leave_stream", ({ streamId }) => {
    socket.leave(streamId);
    const stream = liveStreams.find(s => s.streamId === streamId);
    if (stream && stream.viewersCount > 0) { stream.viewersCount -= 1; io.to(streamId).emit("stream_viewers_updated", { count: stream.viewersCount }); }
  });
  socket.on("send_stream_chat", ({ streamId, text, senderName }) => {
    if (!streamId || !text) return;
    io.to(streamId).emit("receive_stream_chat", { id: Math.random().toString(), senderName: senderName || "दर्शक", text, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) });
  });
  ...
  socket.on("report_partner", ({ reason }) => {
    ...
    if (roomId) {
      dbData.incidents.unshift({ id: Math.random().toString(), room: roomId, user: socket.id.slice(0, 5) + "***", reason: reason || "यूज़र द्वारा रिपोर्ट किया गया", time: new Date().toLocaleTimeString() });
...
server.listen(PORT, () => { console.log(`>>> Sukoon V13 Master Active on port ${PORT}`); });
